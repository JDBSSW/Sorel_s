...
SESSION = start()
engine = create_engine("Sore")
BASE.metadata.bind = engine
BASE.metadata.create_all(engine)


class Jmthon_GlobalCollection(BASE):
    __tablename__ = "Sore_globalcollection"
    ...
    def __repr__(self):
        return "< Sorel Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Sorel_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Sorel_GlobalCollection.__table__.create(checkfirst=True)

Sorel_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with Sorel_GLOBALCOLLECTION:
        keyword_items = Sorel_GlobalCollection(keywoard, tuple(contents))
...
def del_keyword_collectionlist(keywoard):
    with Sorel_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Sorel_GlobalCollection.keywoard)
            .filter(Sorel_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(
            Sorel_GlobalCollection.keywoard).distinct().all()
